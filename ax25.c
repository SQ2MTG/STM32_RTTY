...
void ax25_send_header(ax25_t *packet, const char *callsign, uint8_t ssid, const char *path, uint16_t preamble)
{
	...
	ax25_send_path(packet, APRS_DEST_CALLSIGN, APRS_DEST_SSID, 0);		// Destination callsign
	//ax25_send_path(packet, callsign, ssid, path[0] == 0 || path);	// Source callsign
	ax25_send_path(packet, callsign, ssid, 0);	// Source callsign
	// Parse path
	for(i=0, j=0; (path[i-1] != 0 || i == 0) && path; i++) {
